...
export default function Dashboard() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 md:space-y-6">
      <div>
        <h1 className="text-xl md:text-2xl font-bold text-foreground">Panel Principal</h1>
          ...
          <Link key={s.label} to={s.href}>
            <Card className="hover:shadow-md transition-shadow cursor-pointer">
              <CardContent className="flex items-center gap-3 md:gap-4 p-3 md:p-5">
                <div className={`p-2 md:p-3 rounded-lg bg-accent ${s.color}`}>
        ...
        <Card>
          ...
          <CardContent className="space-y-1 md:space-y-2">
            {upcomingError ? (
              ...
              upcoming.map((a, i) => (
                <div key={a.id || i} className="flex items-center gap-2 md:gap-3 p-1.5 md:p-2 rounded-md hover:bg-accent/50 transition-colors">
                  <span className="text-[10px] md:text-xs font-mono font-medium text-primary w-10 md:w-12">{a.hora}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs md:text-sm font-medium text-foreground truncate">{a.paciente}</p>
                    <p className="text-[10px] md:text-xs text-muted-foreground truncate">{a.medico} - {a.tipo}</p>
                  </div>
                </div>
              ))
            ) : (
        ...
        <Card>
          ...
          <CardContent className="space-y-1 md:space-y-2">
            {activityError ? (
              ...
              activity.map((a, i) => (
                <div key={a.id || i} className="flex items-center gap-2 md:gap-3 p-1.5 md:p-2 rounded-md hover:bg-accent/50 transition-colors">
                  <span className="text-[10px] md:text-xs font-mono text-muted-foreground w-10 md:w-12">{a.hora}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs md:text-sm font-medium text-foreground truncate">{a.accion}</p>
                    <p className="text-[10px] md:text-xs text-muted-foreground truncate">{a.detalle}</p>
                  </div>
                </div>
              ))
            ) : (
