...
export function useAuditLog(filters: AuditLogFilters, page = 1, limit = 25) {
  return useQuery({
    queryKey: ["audit-log", filters, page, limit],
    queryFn: () => {
      ...
      return api.getPaginated<AuditLogEntry>(`/api/v1/audit-log?${params.toString()}`);
    },
  });
}
